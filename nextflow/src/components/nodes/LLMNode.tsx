...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numMatch) {
      elements.push(
        <div key={i} style={{ display: "flex", gap: 6, paddingLeft: 4, margin: "2px 0" }}>
          <span style={{ color: "#4a4a5e", fontSize: 10, fontWeight: 600, minWidth: 14 }}>{numMatch[1]}.</span>
          <span style={{ fontSize: 11, color: "#b4b4c8", lineHeight: 1.5 }}>{formatInline(line.slice(numMatch[0].length))}</span>
        </div>
      );
      continue;
    }
...
function formatInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    else if (match[5]) parts.push(<code key={match.index} style={{ background: "#16161f", border: "1px solid #1e1e2a", borderRadius: 4, padding: "1px 5px", fontSize: 10, color: "#a78bfa", fontFamily: "'SF Mono', monospace" }}>{match[6]}</code>);
    ...
    <>
      <div style={{ marginTop: 6 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: "#4a4a5e", textTransform: "uppercase", letterSpacing: "0.08em" }}>Response</span>
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      >
        ...
        {/* Modal footer */}
        <div style={{ padding: "10px 20px", borderTop: "1px solid #1c1c28", flexShrink: 0, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
