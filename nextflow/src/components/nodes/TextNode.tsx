...
export default memo(function TextNode({ id, data, selected }: NodeProps) {
  ...
  const d = data as any;
  return (
    <BaseNode id={id} type="text" label={d.label || "Text Node"} accentColor="#6366f1" icon={<Type className="w-3.5 h-3.5" />} isRunning={d.isRunning} runStatus={d.runStatus} selected={selected}>
