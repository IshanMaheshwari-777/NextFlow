...
function renderMarkdown(text: string): React.ReactNode[] {
  ...
  for (let i = 0; i < lines.length; i++) {
      ...
      if (!inCodeBlock) {
        ...
        elements.push(
          <pre key={`code-${i}`} style={{
            background: "#0a0a12", border: "1px solid #1e1e2a", borderRadius: 8,
            ...
            {codeLang && <span style={{ fontSize: 9, color: "#4a4a5e", display: "block", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.05em" }}>{codeLang}</span>}
            {codeBuffer.join("\n")}
          </pre>
        );
        continue;
    ...
    // Headers
    if (line.startsWith("### ")) { elements.push(<div key={i} style={{ fontSize: 12, fontWeight: 700, color: "#d4d4e0", margin: "8px 0 4px" }}>{formatInline(line.slice(4))}</div>); continue; }
    if (line.startsWith("## ")) { elements.push(<div key={i} style={{ fontSize: 13, fontWeight: 700, color: "#e4e4ed", margin: "8px 0 4px" }}>{formatInline(line.slice(3))}</div>); continue; }
    if (line.startsWith("# ")) { elements.push(<div key={i} style={{ fontSize: 14, fontWeight: 700, color: "#e4e4ed", margin: "8px 0 4px" }}>{formatInline(line.slice(2))}</div>); continue; }
    ...
    if (line.match(/^[\-\*]\s/)) {
      elements.push(
        <div key={i} style={{ display: "flex", gap: 6, paddingLeft: 4, margin: "2px 0" }}>
          ...
          <span style={{ fontSize: 11, color: "#b4b4c8", lineHeight: 1.5 }}>{formatInline(line.slice(2))}</span>
        </div>
      );
      continue;
    }
    ...
    // Regular paragraph
    elements.push(<div key={i} style={{ fontSize: 11, color: "#b4b4c8", lineHeight: 1.6, margin: "1px 0" }}>{formatInline(line)}</div>);
  }
...
function formatInline(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1]) parts.push(<strong key={match.index} style={{ color: "#e4e4ed", fontWeight: 600 }}>{match[2]}</strong>);
    else if (match[3]) parts.push(<em key={match.index} style={{ color: "#3b82f6" }}>{match[4]}</em>);
    else if (match[5]) parts.push(<code key={match.index} style={{ background: "#16161f", border: "1px solid #1e1e2a", borderRadius: 4, padding: "1px 5px", fontSize: 10, color: "#3b82f6", fontFamily: "'SF Mono', monospace" }}>{match[6]}</code>);
    ...
    <>
      <div style={{ marginTop: 6 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: "#4a4a5e", textTransform: "uppercase", letterSpacing: "0.08em" }}>Enhanced Prompt</span>
          <div style={{ display: "flex", gap: 4 }}>
            <button type="button" onClick={handleCopy} onMouseDown={e => e.stopPropagation()} style={{
              ...
              background: copied ? "rgba(52,211,153,0.12)" : "#16161f", color: copied ? "#34d399" : "#8b8b9e",
        ...
        <div
          ...
          style={{
            background: "#0c0c14", border: "1px solid #1e1e2a", borderRadius: 10,
            padding: "10px 12px", maxHeight: 100, overflow: "hidden",
            position: "relative", cursor: "pointer",
          }}
        >
          <div style={{ fontSize: 11, color: "#b4b4c8", lineHeight: 1.6, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
            {preview}
          </div>
          {text.length > 120 && (
            <div style={{
    ...
    >
      <div
        ...
        style={{
          ...
          background: "#0e0e14", border: "1px solid #1c1c28", borderRadius: 16,
      ...
      >
        <div style={{
          ...
          padding: "14px 20px", borderBottom: "1px solid #1c1c28", flexShrink: 0,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: "#e4e4ed" }}>Enhanced Prompt</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={onClose} style={{
              ...
              background: "transparent", border: "1px solid transparent", color: "#4a4a5e", cursor: "pointer", transition: "all 150ms ease",
            }} onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; e.currentTarget.style.color = "#8b8b9e"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#4a4a5e"; }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
            </button>
          </div>
        </div>
        <div style={{ padding: "24px", overflowY: "auto", flex: 1, scrollbarWidth: "thin", scrollbarColor: "#1c1c28 transparent" }}>
